...
void Thermocouple::begin(uint8_t csPin) {
    ...
    // Initial settling read (discard)
    delay(100);
    _sensor->readCelsius();
}

void Thermocouple::update() {
    if (!_initialized || _sensor == nullptr) return;
    ...
    if (fault) {
        ...
        if (fault & MAX31855_FAULT_OPEN) {
            _status = TCStatus::OPEN_CIRCUIT;
        } else if (fault & MAX31855_FAULT_SHORT_GND) {
            _status = TCStatus::SHORT_GND;
        } else if (fault & MAX31855_FAULT_SHORT_VCC) {
            _status = TCStatus::SHORT_VCC;
        } else {
            _status = TCStatus::READ_ERROR;
        }
        return;
    ...
    _tempC = (float)tempC;
    _tempF = celsiusToFahrenheit(_tempC);
